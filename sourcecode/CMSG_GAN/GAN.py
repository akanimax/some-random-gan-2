...
class Generator(th.nn.Module):
    ...
    def __init__(self, depth=7, latent_size=512,
                 use_coord_conv=True, use_upsampling=False,
        ...
        :param use_coord_conv: whether to use coord_conv
                               for the generation [default=True].
        :param use_upsampling: whether to use upsampling or ConvT
                               [default = False]
    ...
    def turn_on_spectral_norm(self):
        ...
        for module in self.layers:
            if self.use_coord_conv:
                module.conv_1.conv = spectral_norm(module.conv_1.conv)
                module.conv_2.conv = spectral_norm(module.conv_2.conv)
            else:
    ...
    def turn_off_spectral_norm(self):
        ...
        for module in self.layers:
            if self.use_coord_conv:
                remove_spectral_norm(module.conv_1.conv)
                remove_spectral_norm(module.conv_2.conv)
            else:
...
class Discriminator(th.nn.Module):
    ...
    def __init__(self, depth=7, feature_size=512, use_coord_conv=True, use_spectral_norm=True):
        """
        ...
        :param use_coord_conv: whether to use coord_conv layers for discrimination
                               [default = True]
...
class CMSG_GAN:
    """ Unconditional CoordConv-MSG_GAN
        ...
        args:
            ...
            gen_coord_conv: whether to use coord_conv for generator
            dis_coord_conv: whether to use coord_conv for discriminator
            use_spectral_norm: whether to use spectral normalization to the convolutional
                               blocks.
            use_upsampling: whether to use upsampling or ConvT for generator
            device: device to run the GAN on (GPU / CPU)
    """

    def __init__(self, depth=7, latent_size=512, gen_coord_conv=True,
                 dis_coord_conv=True, use_spectral_norm=True,
                 use_upsampling=False, device=th.device("cpu")):
        """ constructor for the class """
        from torch.nn import DataParallel

        self.gen = Generator(depth, latent_size,
                             use_coord_conv=gen_coord_conv,
                             use_upsampling=use_upsampling,
                             use_spectral_norm=use_spectral_norm).to(device)
        self.dis = Discriminator(depth, latent_size,
                                 use_coord_conv=dis_coord_conv,
                                 use_spectral_norm=use_spectral_norm).to(device)
